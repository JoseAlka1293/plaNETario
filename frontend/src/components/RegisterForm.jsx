...
function RegisterForm({ onSwitch, darkMode }) {
  const [formData, setFormData] = useState({ nombre: '', email: '', contraseña: '' });
  const [errors, setErrors] = useState({ nombre: '', email: '', contraseña: '' });
  const [focusedField, setFocusedField] = useState('');
  const [passwordStrength, setPasswordStrength] = useState(0);

  const validateNombre = (nombre) => {
    if (nombre.length < 5) return 'El nombre debe tener al menos 5 caracteres';
    if (!/[A-Z]/.test(nombre)) return 'El nombre debe contener al menos una mayúscula';
    return '';
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (nombreError || emailError || contraseñaError) {
      setErrors({ nombre: nombreError, email: emailError, contraseña: contraseñaError });
      return;
    }
    try {
  ...
  return (
    <div
      className={`backdrop-blur-lg p-8 rounded-2xl shadow-lg max-w-[700px] w-[26rem] animate-fadeIn ${
        darkMode
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div className="relative flex flex-col">
          <label
            className={`text-sm mb-1 transition-transform duration-300 ${
              focusedField === 'nombre' ? 'animate-wave' : ''
            }`}
          >
            Nombre
          </label>
          ...
          />
          {errors.nombre && (
            <div className="absolute right-[-210px] top-1/2 transform -translate-y-1/2 bg-red-500 text-white text-xs p-2 rounded-lg shadow-lg w-48">
              {errors.nombre}
        ...
        <p className="text-sm text-center mt-4">
          ...
          >
            <span className={`${darkMode ? 'text-indigo-300' : 'text-indigo-500'}`}>
              Login
            </span>
          </button>
        </p>
